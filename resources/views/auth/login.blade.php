...
@section('contenido')
    <div class="md:flex justify-center md:gap-10 md:items-center">
        ...
        <div class="md:w-4/12 bg-white p-6 rounded-xl shadow-xl">
            <form method="POST" action="{{ route('login')}}" novalidate> {{-- Action es la url a la que queremos mandar la informacion y hay que especificar el METHOD --}}
                @csrf {{-- con esto evitamos ataques y nos crea un campo oculto --}}

                {{-- Mostramos un mensaje de error si el usuario no mete credenciales correctas --}}
                @if (session('mensaje'))
                ...
                <div class="mb-5">
                    <input type="checkbox" name="remember"> <label class="text-sm text-gray-500 "> Mantener la sesión abierta </label>
                </div>
                {{-- Boton --}}
                <input 
                    type="submit"
